...
fn main() {
    ...
    let mut cards_with_quantity = cards.clone();
    for (i, card) in cards.iter().enumerate() {
      if card.points > 0 {
        for j in i+1..i + 1 + card.points as usize {
          println!("Card {} has won {} copies of card {}", card.id, card.quantity, cards_with_quantity[j].id);
          cards_with_quantity[j].quantity += 1 * cards_with_quantity[i].quantity;
        }
      }
    }

    cards_with_quantity.iter().for_each(|card| println!("{:?}", card));
    let sum = cards_with_quantity.iter().fold(0, |acc, card| acc + card.quantity);
