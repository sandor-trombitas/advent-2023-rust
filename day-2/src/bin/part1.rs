...
fn parse_input (input: &str) -> Vec<Game> {
    let games: Vec<Game> = input.lines().map(|game| {
        let game_id = game.split(": ").next().unwrap().split(' ').last().unwrap().parse::<i32>().unwrap();
        let parsed_draws: Vec<Draw> = game.split(": ").last().unwrap().split("; ").map(|draws| {
            let mut draw_game = Draw {
                is_possible: true,
                red: 0,
                green: 0,
                blue: 0
            ...
            draws.split(", ").for_each(|draw| {
                let cube = draw.trim_start().split(' ').last().unwrap();
                let amount = draw.trim_start().split(' ').next().unwrap().parse::<i32>().unwrap();

                match cube {
                    "red" => {
                        if amount > MAX_RED {
                            draw_game.is_possible = false;
                        }
                        if amount > draw_game.red {
                            draw_game.red = amount;
                        }
                    }
                    "green" => {
                        if amount > MAX_GREEN {
                        ...
                        if amount > draw_game.green {
                            draw_game.green = amount;
                        }
                    }
                    "blue" => {
                        if amount > MAX_BLUE {
                        ...
                        if amount > draw_game.blue {
                            draw_game.blue = amount;
                        }
                    }
                    _ => {}
                };
        ...
        let is_possible = parsed_draws.iter().fold(true, | acc, draw | acc & draw.is_possible );

        return Game { id: game_id, draws: parsed_draws, is_possible: is_possible};
    }).collect();
...
";
    ...
    let sum_possible = games.iter().fold(0, |acc, game| {
        if game.is_possible {
            return acc + game.id;
        }
    ...
    let fewest_cubes: Vec<(i32, i32, i32)> = games.iter().map(|game| {
        let max_reds = game.draws.iter().fold(0, | acc, draw | {
            if draw.red > acc {
                return draw.red;
            }
            return acc;
        ...
        let max_greens = game.draws.iter().fold(0, | acc, draw | {
            if draw.green > acc {
                return draw.green;
            }
            return acc;
        ...
        let max_blues = game.draws.iter().fold(0, | acc, draw | {
            if draw.blue > acc {
                return draw.blue;
            }
            return acc;
    ...
    let powers: Vec<i32> = fewest_cubes.into_iter().map(| (red, green, blue) | {

        println!("rgb is {} {} {}", red, green, blue);
        println!("Power is {}", red * green * blue);
        return red * green * blue;
    }).collect();

    let sum_powers = powers.into_iter().fold(0, |acc, power| acc + power);
